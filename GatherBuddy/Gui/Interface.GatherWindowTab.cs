...
public partial class Interface
{
    ...
    {
        if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Copy.ToIconString(), IconButtonSize, Localize.Text("Copy current gather window preset to clipboard."),
                _gatherWindowCache.Selector.Current == null, true))
        {
            ...
            try
            {
                ...
                Communicator.PrintClipboardMessage(Localize.Text("Gather window preset "), preset.Name);
            }
            catch (Exception e)
            {
                Communicator.PrintClipboardMessage(Localize.Text("Gather window preset "), preset.Name, e);
            }
        }

        if (ImGuiUtil.DrawDisabledButton(Localize.Label("Create Alarms"), Vector2.Zero, Localize.Text("Create a new Alarm Group from this gather window preset."), _gatherWindowCache.Selector.Current == null))
        {
    ...
    private void DrawGatherWindowPreset(GatherWindowPreset preset)
    {
        ...
        if (ImGui.Checkbox(Localize.Label("Enabled##preset"), ref tmp) && tmp != preset.Enabled)
        ...
        for (var i = 0; i < preset.Items.Count; ++i)
        {
            ...
            if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Trash.ToIconString(), IconButtonSize, Localize.Text("Delete this item from the preset..."), false,
                    true))
        ...
        if (ImGuiUtil.DrawDisabledButton(FontAwesomeIcon.Plus.ToIconString(), IconButtonSize,
                Localize.Text("Add this item at the end of the preset, if it is not already included..."),
    ...
    private void DrawGatherWindowTab()
    {
        ...
        using var tab = ImRaii.TabItem(Localize.Label("Gather Window"));

        ImGuiUtil.HoverTooltip(
            Localize.Text("Config window too big? Why can't you hold all this information?\nPrepare a small window with only those items that actually interest you!"));
        ...
        ItemDetailsWindow.Draw(Localize.Text("Preset Details"), DrawGatherWindowPresetHeaderLine, () =>
        {
